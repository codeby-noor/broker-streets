...
function LocationDetailsPage() {
    ...
    return (
      <div className="min-h-screen bg-background px-6 py-20 text-text">
        <div className="mx-auto max-w-5xl rounded-[32px] border border-slate-200 bg-white p-10 text-center shadow-card">
          <p className="eyebrow">{t('location.notFound')}</p>
          <h1 className="mt-4 text-3xl font-semibold text-ink">{t('location.notFoundDesc')}</h1>
          <button type="button" onClick={() => navigate('/home')} className="mt-8 inline-flex rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white">{t('common.backToHome')}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background text-text">
      <section className="relative overflow-hidden bg-slate-900 text-white">
        ...
        <div className="relative mx-auto flex min-h-[520px] max-w-7xl flex-col justify-end px-6 py-24 sm:px-10 lg:px-12">
          <div className="max-w-3xl">
            <p className="eyebrow text-accentSoft">{t('home.hero.marketplace')}</p>
            <h1 className="mt-4 text-4xl font-semibold leading-tight sm:text-5xl">{t(location.name)}, {t(location.district)}</h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-white/80">{location.description}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur-sm">{location.activeListings} {t('home.verifiedListingTitle')}</span>
              <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur-sm">{location.verifiedSellers} {t('sell.sellerVerified')}</span>
              <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur-sm">{t('home.hero.marketplace')}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 lg:px-12">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
            <p className="eyebrow">{t('location.overview')}</p>
            <h2 className="mt-4 text-3xl font-semibold text-ink">{t('location.whyActive')} {t(location.name)}</h2>
            <p className="mt-4 text-base leading-8 text-slate-600">{location.description}</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {[
                { label: t('common.district'), value: t(location.district) },
                { label: t('common.propertyType'), value: location.propertyTypes.map((type) => t(type)).join(' • ') },
                { label: t('location.totalListings'), value: location.activeListings },
                { label: t('location.agriListings'), value: agriculturalCount },
                { label: t('location.nonAgriListings'), value: nonAgriculturalCount },
                { label: t('location.avgPrice'), value: location.averagePrice },
                { label: t('location.highPrice'), value: location.highestPrice },
                { label: t('location.lowPrice'), value: location.lowestPrice },
              ].map((stat) => (
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
            <p className="eyebrow">{t('location.quickStats')}</p>
            <div className="mt-6 space-y-4">
              {[
                [t('sell.sellerVerified'), location.verifiedSellers],
                [t('location.activeBuyers'), location.activeBuyers],
                [t('sell.recentlySold'), location.propertiesSold],
                [t('location.activeListings'), location.activeListings],
                [t('location.avgPrice'), location.averagePrice],
              ].map(([label, value]) => (
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 lg:px-12">
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="eyebrow">{t('buy.filterHeading')}</p>
              <h2 className="mt-3 text-3xl font-semibold text-ink">{t('location.findLandIn')} {t(location.name)}</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('common.search')} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none" />
              <select value={propertyType} onChange={(event) => setPropertyType(event.target.value)} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none">
                ...
                <option value="Agricultural">{t('buyerForm.agriculturalLand')}</option>
                <option value="Non-Agricultural">{t('buyerForm.nonAgriculturalLand')}</option>
              </select>
              <input value={budget} onChange={(event) => setBudget(event.target.value)} placeholder={t('buy.price')} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none" />
              <input value={area} onChange={(event) => setArea(event.target.value)} placeholder={t('buy.landArea')} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none" />
              <select value={sort} onChange={(event) => setSort(event.target.value)} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none">
                <option value="latest">{t('dropdown.newest')}</option>
                <option value="price">{t('buy.price')}</option>
              </select>
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
            ...
            )) : <div className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 p-8 text-sm text-slate-600 lg:col-span-3">{t('buy.noProperties')}</div>}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 lg:px-12">
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
          <p className="eyebrow">{t('location.whyInvest')}</p>
          <h2 className="mt-3 text-3xl font-semibold text-ink">{t('location.investDesc')}</h2>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 lg:px-12">
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
            <p className="eyebrow">{t('location.highlights')}</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
            <p className="eyebrow">{t('propertyDetails.propertyLocation')}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 lg:px-12">
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-card">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="eyebrow">{t('home.readySubtitle')}</p>
              <h2 className="mt-3 text-3xl font-semibold text-ink">{t('location.connectWith')} {t(location.name)}</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => navigate('/buy')} className="rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white">{t('home.hero.browse')}</button>
              <button type="button" onClick={() => navigate('/sell')} className="rounded-full border border-primary px-6 py-3 text-sm font-semibold text-primary">{t('home.hero.postLand')}</button>
            </div>
          </div>
          <div className="mt-8 rounded-[24px] border border-slate-200 bg-slate-50 p-6">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">{t('location.relatedLocations')}</p>
            <div className="mt-4 flex flex-wrap gap-3">
              {location.relatedLocations.map((item) => (
                <button key={item} type="button" onClick={() => navigate(`/location/${item.toLowerCase().replace(/\s+/g, '-')}`)} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">{t(item)}</button>
              ))}
            </div>
          </div>
        </div>
