...
function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link to={hideNav ? '/' : '/home'} className="flex items-center gap-3">
          <img src={logo} alt="Broker Streets logo" className="h-12 w-auto object-contain" />
        ...
        <button
          ...
          className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-800 shadow-sm transition hover:bg-slate-50 md:hidden"
        >
      ...
      {menuOpen && !hideNav ? (
        <>
          ...
          <div className="absolute inset-x-0 top-full z-40 border-t border-slate-200 bg-white shadow-xl md:hidden transform transition-transform duration-300">
              ...
              <div className="space-y-4">
                {navItems.map((item) => (
                  <button
                    ...
                    className="w-full rounded-3xl px-5 py-4 text-left text-base font-semibold text-slate-800 transition hover:bg-slate-50"
                  >
                    {item.label}
                  </button>
                ))}
