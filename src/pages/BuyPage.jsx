...
import { amenities, propertyTypes, sampleProperties, states } from '../utils/data';
...
function BuyPage() {
  ...
  const filtered = useMemo(() => {
    let list = sampleProperties.filter((property) => {
      const priceValue = Number(property.price.replace(/[^0-9]/g, ''));
      const matchesCity = city === 'All cities' || property.city === city || property.location === city;
      ...
      const areaValue = Number(property.area.replace(/[^0-9]/g, ''));
      ...
      const matchesParking = !parking || property.parking;
      const matchesReady = !readyToMove || property.readyToMove;
      const matchesAmenities = selectedAmenities.every((item) => property.amenities.includes(item));
      const normalizedQuery = query.trim().toLowerCase();
      const matchesQuery = !normalizedQuery || `${property.title} ${property.location} ${property.city} ${property.type}`.toLowerCase().includes(normalizedQuery);
      return matchesCity && matchesType && matchesPrice && matchesBhk && matchesBathrooms && matchesArea && matchesStatus && matchesFurnished && matchesParking && matchesReady && matchesAmenities && matchesQuery;
    });

    if (sort === 'price_asc') list = list.sort((a, b) => Number(a.price.replace(/[^0-9]/g, '')) - Number(b.price.replace(/[^0-9]/g, '')));
    if (sort === 'price_desc') list = list.sort((a, b) => Number(b.price.replace(/[^0-9]/g, '')) - Number(a.price.replace(/[^0-9]/g, '')));
    if (sort === 'area_asc') list = list.sort((a, b) => Number(a.area.replace(/[^0-9]/g, '')) - Number(b.area.replace(/[^0-9]/g, '')));
    if (sort === 'newest') list = list.sort((a, b) => b.id.localeCompare(a.id, undefined, { numeric: true }));
    return list;
  }, [city, type, minPrice, maxPrice, bhk, bathrooms, area, status, furnished, parking, readyToMove, selectedAmenities, query, sort]);
  ...
  const filters = (
    <div className="space-y-7">
      ...
      <div><label className="field-label">Location</label><select value={city} onChange={(event) => { setCity(event.target.value); setPage(1); }} className="field-control"><option>All cities</option>{selectedState.cities.map((item) => <option key={item}>{item}</option>)}</select></div>
      <div><label className="field-label">Property type</label><select value={type} onChange={(event) => { setType(event.target.value); setPage(1); }} className="field-control"><option>All types</option>{propertyTypes.map((item) => <option key={item}>{item}</option>)}</select></div>
      ...
      <div><p className="field-label">Amenities</p><div className="space-y-3">{amenities.map((item) => <label key={item} className="flex items-center gap-3 text-sm text-muted"><input type="checkbox" checked={selectedAmenities.includes(item)} onChange={(event) => { setSelectedAmenities((current) => event.target.checked ? [...current, item] : current.filter((value) => value !== item)); setPage(1); }} className="h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary" />{item}</label>)}</div></div>
